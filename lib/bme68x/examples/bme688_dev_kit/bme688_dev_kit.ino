...
void setup(void)
{
	...
	for (uint8_t i = 0; i < N_KIT_SENS; i++) {
		...
		if (bme[i].checkStatus()) {
			Serial.println("Initializing sensor " + String(i) + " failed with error " +
				       bme[i].statusString());
			panicLeds();
		}
	}
				...
				do {
					...
					if (sensorData[i].status & BME68X_NEW_DATA_MSK) {
						...
						if (indexDiff > 1) {
							...
							Serial.println(
								"Skip I:" + String(i) + ", DIFF:" +
								String(indexDiff) + ", MI:" +
								String(sensorData[i].meas_index) +
								", LMI:" +
								String(lastMeasindex[i]) + ", S:" +
								String(sensorData[i].status, HEX));
							panicLeds();
						}
