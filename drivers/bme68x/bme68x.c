/* bme68x.c - Driver for Bosch Sensortec's BME68X temperature, pressure,
 * humidity and gas sensor
 *
 * https://www.bosch-sensortec.com/bst/products/all_products/bme68x
 */
...
LOG_MODULE_REGISTER(bme68x, CONFIG_SENSOR_LOG_LEVEL);

static inline int bme68x_bus_check(const struct device *dev)
{
...
static inline int bme68x_reg_read(const struct device *dev, uint8_t start, uint8_t *buf, int size)
{
	const struct bme68x_config *config = dev->config;
...
static inline int bme68x_reg_write(const struct device *dev, uint8_t reg, uint8_t val)
{
	const struct bme68x_config *config = dev->config;
...
static void bme68x_calc_temp(struct bme68x_data *data, uint32_t adc_temp)
{
...
static void bme68x_calc_press(struct bme68x_data *data, uint32_t adc_press)
{
...
static void bme68x_calc_humidity(struct bme68x_data *data, uint16_t adc_humidity)
{
...
static int bme68x_sample_fetch(const struct device *dev, enum sensor_channel chan)
{
	...
	ret = bme68x_reg_read(dev, BME68X_REG_FIELD0, buff, size);
	if (ret < 0) {
		return ret;
	}

	data->new_data = buff[0] & BME68X_MSK_NEW_DATA;
	data->heatr_stab = buff[14] & BME68X_MSK_HEATR_STAB;
	...
	ret = bme68x_reg_write(dev, BME68X_REG_CTRL_MEAS, BME68X_CTRL_MEAS_VAL);
	if (ret < 0) {
		return ret;
	}
...
static int bme68x_read_compensation(const struct device *dev)
{
	struct bme68x_data *data = dev->data;
	uint8_t buff[BME68X_LEN_COEFF_ALL];
	int err = 0;

	err = bme68x_reg_read(dev, BME68X_REG_COEFF1, buff, BME68X_LEN_COEFF1);
	if (err < 0) {
		return err;
	}

	err = bme68x_reg_read(dev, BME68X_REG_COEFF2, &buff[BME68X_LEN_COEFF1], BME68X_LEN_COEFF2);
	if (err < 0) {
		return err;
	}

	err = bme68x_reg_read(dev, BME68X_REG_COEFF3, &buff[BME68X_LEN_COEFF1 + BME68X_LEN_COEFF2],
			      BME68X_LEN_COEFF3);
	if (err < 0) {
		return err;
	}
	...
	data->par_t1 = (uint16_t)(BME68X_CONCAT_BYTES(buff[32], buff[31]));
	data->par_t2 = (int16_t)(BME68X_CONCAT_BYTES(buff[1], buff[0]));
	data->par_t3 = (uint8_t)(buff[2]);

	/* Pressure related coefficients */
	data->par_p1 = (uint16_t)(BME68X_CONCAT_BYTES(buff[5], buff[4]));
	data->par_p2 = (int16_t)(BME68X_CONCAT_BYTES(buff[7], buff[6]));
	data->par_p3 = (int8_t)buff[8];
	data->par_p4 = (int16_t)(BME68X_CONCAT_BYTES(buff[11], buff[10]));
	data->par_p5 = (int16_t)(BME68X_CONCAT_BYTES(buff[13], buff[12]));
	data->par_p6 = (int8_t)(buff[15]);
	data->par_p7 = (int8_t)(buff[14]);
	data->par_p8 = (int16_t)(BME68X_CONCAT_BYTES(buff[19], buff[18]));
	data->par_p9 = (int16_t)(BME68X_CONCAT_BYTES(buff[21], buff[20]));
...
static int bme68x_init(const struct device *dev)
{
	struct bme68x_data *data = dev->data;
	int err;

	err = bme68x_bus_check(dev);
	if (err < 0) {
		LOG_ERR("Bus not ready for '%s'", dev->name);
		return err;
	}

	err = bme68x_reg_read(dev, BME68X_REG_CHIP_ID, &data->chip_id, 1);
	if (err < 0) {
		return err;
	}

	if (data->chip_id == BME68X_CHIP_ID) {
		LOG_DBG("BME68X chip detected");
	} else {
		LOG_ERR("Bad BME68X chip id: 0x%x", data->chip_id);
		return -ENOTSUP;
	}

	err = bme68x_read_compensation(dev);
	if (err < 0) {
		return err;
	}

	err = bme68x_reg_write(dev, BME68X_REG_CTRL_HUM, BME68X_HUMIDITY_OVER);
	if (err < 0) {
		return err;
	}

	err = bme68x_reg_write(dev, BME68X_REG_CONFIG, BME68X_CONFIG_VAL);
	if (err < 0) {
		return err;
	}
	...
	// err = bme68x_reg_write(dev, BME68X_REG_GAS_WAIT0,
	// 		       bme68x_calc_gas_wait(BME68X_HEATR_DUR_MS));
	// if (err < 0) {
	// 	return err;
	// }
...
static const struct sensor_driver_api bme68x_api_funcs = {
	.sample_fetch = bme68x_sample_fetch,
	.channel_get = bme68x_channel_get,
};

/* Initializes a struct bme68x_config for an instance on an I2C bus. */
#define BME68X_CONFIG_I2C(inst)                                                                    \
	{                                                                                          \
		.bus.i2c = I2C_DT_SPEC_INST_GET(inst), .bus_io = &bme68x_bus_io_i2c,               \
	}
 ...
 */
#define BME68X_DEFINE(inst)                                                                        \
	static struct bme68x_data bme68x_data_##inst;                                              \
	static const struct bme68x_config bme68x_config_##inst = BME68X_CONFIG_I2C(inst);          \
	SENSOR_DEVICE_DT_INST_DEFINE(inst, bme68x_init, NULL, &bme68x_data_##inst,                 \
				     &bme68x_config_##inst, APPLICATION,                           \
				     CONFIG_SENSOR_INIT_PRIORITY, &bme68x_api_funcs);

/* Create the struct device for every status "okay" node in the devicetree. */
DT_INST_FOREACH_STATUS_OKAY(BME68X_DEFINE)
